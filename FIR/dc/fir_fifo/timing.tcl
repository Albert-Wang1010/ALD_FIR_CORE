...
if {[sizeof_collection [get_ports clk1]] > 0} {
  ...
  # Split setup vs hold uncertainty (fixes small hold violation)
...
###################################################################### 
# Hold Time Fixing - CRITICAL for slow clocks
# Allows synthesis to insert delay cells to fix hold violations
###################################################################### 
# Enable automatic hold fixing for all clocks
set_fix_hold [all_clocks]

# Set realistic max delay for clk1 paths (90% of clock period)
# This gives synthesis headroom to add delay buffers for hold fixing
set_max_delay 90000.0 -from [get_clocks clk1] -to [get_clocks clk1]

# Prioritize meeting timing over area optimization
set_cost_priority -delay

###################################################################### 
# Don't touch CDC synchronizer cells
# CRITICAL: Prevents optimization that could break metastability protection
# The 2-FF synchronizers must remain intact for CDC safety
###################################################################### 
if {[sizeof_collection [get_cells -hierarchical *sync*]] > 0} {
  # Use size_only instead of dont_touch to allow buffer insertion for hold fixing
  # This prevents logic optimization but permits delay cell insertion
